...
function Setup() {
  ...
  const [nativeToken, setNativeToken] = useState(
    supportChain ? supportChain.contracts.FTM_ADDRESS : ""
  );
  const [wNativeToken, setWNativeToken] = useState(
    supportChain ? supportChain.contracts.WFTM_SYMBOL : ""
  );
    ...
    function () {
      ...
      const ssUpdated = () => {
        ...
        if (baseAsset.length > 0) {
          ...
          if (toAssetValue) {
            _toAssetValue = baseAsset.find(
              (token) => token.address === toAssetValue.address
            );
          } else {
            _toAssetValue = baseAsset.find(
              (token) =>
                token.address.toLowerCase() === outputCurrency.toLowerCase()
            );
          }

          if (fromAssetValue) {
            _fromAssetValue = baseAsset.find(
              (token) => token.address === fromAssetValue.address
            );
          } else {
            _fromAssetValue = baseAsset.find(
              (token) =>
                token.address.toLowerCase() === inputCurrency.toLowerCase()
            );
          }
          if (_toAssetValue) {
            ...
            >
              ...
              <div className={classes.line}>
                ...
                {quote?.output?.routeAsset &&
                  quote?.output?.routeAsset.length === 1 && (
                    <>
                      <div
                        className={[
                ...
                {quote?.output?.routeAsset &&
                  quote?.output?.routeAsset.length === 2 && (
                    <>
                      <div
                        className={[
                          classes.routeLinesLeftText,
      ...
      {supportChain && supportChain.id === "82" && (
        ...
        >
          <a
            ...
            href="https://wallet.meter.io/swap"
            target="_blank"
          >
            Gas Station
          </a>
